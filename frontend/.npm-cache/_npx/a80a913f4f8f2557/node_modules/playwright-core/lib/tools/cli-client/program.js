...
async function listSessions(registry, clientInfo, all) {
  if (all) {
    ...
    for (const [workspace, list] of entries)
      await gcAndPrintSessions(clientInfo, list.map((entry) => new import_session.Session(entry)), `${import_path.default.relative(process.cwd(), workspace) || "/"}:`, runningSessions);
