...
本仓库保留两套用途不同的检索层评测：项目提交报告口径为 1081 条、Top-4 召回 88.62%；后续冻结复核 benchmark 为 1000 条、RAG V2 X2 Top-4 chunk recall 81.5%，并公开完整原始证据。二者数据构造与协议不同、不可横向比较，详见 [评测总览](../../evaluation.md)。无论采用哪套检索口径，检索召回率都不等于最终回答正确——正确证据进入上下文之后，语言模型仍可能引入事实错误、过度承诺或越界医疗建议。
